...
import {mainPageStructure} from "./html.js";
...
getClient(DATA_LINK).then(
    result => {
        ...
        //localStorage.removeItem('dataArray'); // РАСКОММЕНТИРОВАТЬ ЧТОБЫ ВРУЧНУЮ ОЧИСТИТЬ КЛЮЧ С ВОШЕДШИМ КЛИЕНТОМ
            ...
            for (let i = 0; i < dataArray.length; i++) {
                if (dataArray[i].isAuthenticate === true) {
                    const alreadySigned = () => {
                        ...
                        let greeting = dataArray[i].greeting !== undefined ? dataArray[i].greeting
                            : 'Hello, ' + dataArray[i].email.split('@')[0] + '! You have 1 unread message.';
                        if (dataArray[i].balance === undefined) {
                            dataArray[i].balance = '$0.00';
                        }
                        let balance = 'Your balance: ' + dataArray[i].balance;
                        let device = 'You are logged in from ' + getDevice() + '.';
                        document.documentElement.innerHTML = mainPageStructure;
                    }
                    alreadySigned();
